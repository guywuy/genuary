<script>
  ...
  const evolveCells = (cellArr) => {
    ...
    cells = cellArr.map( (row, iY) => {
      return row.map( (cell, iX) => {
        ...
        const br = (iY !== numY - 1 && iX !== numX - 1) && cellArr[iY + 1][iX + 1];

        const numLive = left + right + top + bottom + tl + tr + bl + br;

        return (numLive === 2 || numLive === 3) && cell === 1 ? 1 :
        numLive === 3 && cell === 0 ? 1 :
        0;
      } )
    } )
  ...
  const toggleEvolution = () => {
    inProgress ? clearInterval(timer) : timer = setInterval(() => evolveCells(cells), 500);
...
<button on:click={toggleEvolution} class="btn bg-white mg-btm">{inProgress ? 'Pause' : 'Start'} evolution</button>

<svg viewbox={`0 0 ${numX * 10} ${numY * 10}`} width={numX*10} height={numY*10} preserveAspectRatio="xMidYMid meet">
  {#each cells as row, i}
    {#each row as cell, num}
      <rect x={num*10} y={i*10} width={10} height={10} opacity={'1'} fill={cell ? 'white' : 'black'}/>
    {/each}
  {/each}
</svg>
