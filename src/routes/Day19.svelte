<script>
  ...
  const removeParticle = id => particles = [...particles.filter( p => p.id !== id)];

  const createParticle = () => withLog('creating particle', particles.push(new Particle(uuidv4(), Math.random() * width, Math.random() * height, Math.floor(Math.random() * 1000))));
